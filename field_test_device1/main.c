...
unsigned int encode_sensors(uint8_t *payload, const unsigned int len) {
    ...
    // todo: convert temp to K
    temperature+=27300;

    
    CoreconfValueT* temperatureValueMap = createCoreconfHashmap();
    CoreconfValueT* temperatureValue = createCoreconfInteger(temperature);
    insertCoreconfHashMap(temperatureValueMap->data.map_value, 1003, temperatureValue);
    nanocbor_encoder_init(&encoder, cbor_buffer, cbor_buffer_size);
    coreconfToCBOR(temperatureValueMap, &encoder);
    freeCoreconf(temperatureValueMap, true);
